<?php
// created: 2019-08-01 13:20:13
$dictionary["frnt_fornitori_azienda_cases_1"] = array (
  'true_relationship_type' => 'one-to-many',
  'from_studio' => true,
  'relationships' => 
  array (
    'frnt_fornitori_azienda_cases_1' => 
    array (
      'lhs_module' => 'frnt_fornitori_azienda',
      'lhs_table' => 'frnt_fornitori_azienda',
      'lhs_key' => 'id',
      'rhs_module' => 'Cases',
      'rhs_table' => 'cases',
      'rhs_key' => 'id',
      'relationship_type' => 'many-to-many',
      'join_table' => 'frnt_fornitori_azienda_cases_1_c',
      'join_key_lhs' => 'frnt_fornitori_azienda_cases_1frnt_fornitori_azienda_ida',
      'join_key_rhs' => 'frnt_fornitori_azienda_cases_1cases_idb',
    ),
  ),
  'table' => 'frnt_fornitori_azienda_cases_1_c',
  'fields' => 
  array (
    0 => 
    array (
      'name' => 'id',
      'type' => 'varchar',
      'len' => 36,
    ),
    1 => 
    array (
      'name' => 'date_modified',
      'type' => 'datetime',
    ),
    2 => 
    array (
      'name' => 'deleted',
      'type' => 'bool',
      'len' => '1',
      'default' => '0',
      'required' => true,
    ),
    3 => 
    array (
      'name' => 'frnt_fornitori_azienda_cases_1frnt_fornitori_azienda_ida',
      'type' => 'varchar',
      'len' => 36,
    ),
    4 => 
    array (
      'name' => 'frnt_fornitori_azienda_cases_1cases_idb',
      'type' => 'varchar',
      'len' => 36,
    ),
  ),
  'indices' => 
  array (
    0 => 
    array (
      'name' => 'frnt_fornitori_azienda_cases_1spk',
      'type' => 'primary',
      'fields' => 
      array (
        0 => 'id',
      ),
    ),
    1 => 
    array (
      'name' => 'frnt_fornitori_azienda_cases_1_ida1',
      'type' => 'index',
      'fields' => 
      array (
        0 => 'frnt_fornitori_azienda_cases_1frnt_fornitori_azienda_ida',
      ),
    ),
    2 => 
    array (
      'name' => 'frnt_fornitori_azienda_cases_1_alt',
      'type' => 'alternate_key',
      'fields' => 
      array (
        0 => 'frnt_fornitori_azienda_cases_1cases_idb',
      ),
    ),
  ),
);