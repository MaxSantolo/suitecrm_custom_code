...
//carrys forward custom lead fields to contacts, accounts, opportunities during Lead Conversion
    ...
    public function get_related_notes()
    {
        ...
        $return_array['where']=" WHERE notes.deleted = 0 and emails.parent_id = '$this->id'"; //and emails.parent_id = '$this->id'
    ...
    public function get_quotes_notes_accounts()
    {
        $return_array['select'] = "SELECT notes.id";
        $return_array['from']="FROM notes  ";
        $return_array['where']=" where notes.deleted = 0 and emails.deleted = 0 and aos_quotes.deleted = 0
                                 and leads_aos_quotes_1leads_ida = '$this->id'"; //and emails.parent_id = '$this->id'
        $return_array['join'] = " join emails on notes.parent_id = emails.id
                                  join aos_quotes ON emails.parent_id = aos_quotes.id
                                  join leads_aos_quotes_1_c ON aos_quotes.id = leads_aos_quotes_1_c.leads_aos_quotes_1aos_quotes_idb";
        $return_array['join_tables'][0] = '';
