...
class Account extends Company implements EmailInterface {
	...
	*/
	...
	//legge gli allegati separati delle quotazioni legate all'account
    public function get_quotes_notes_accounts()
    {
        $return_array['select'] = "SELECT notes.id";
        $return_array['from']="FROM notes  ";
        $return_array['where']=" where aos_quotes.deleted = 0 and emails.deleted = 0 and notes.deleted = 0 and aos_quotes.billing_account_id = '$this->id'";
        $return_array['join'] = " join emails on notes.parent_id = emails.id
                                  join aos_quotes on emails.parent_id = aos_quotes.id";
    ...
    //note che vengono da allegati mail quotazione di contatti dell'azienda
    public function get_notes_quotes_leads() {

            $return_array['select'] = " SELECT notes.id";
            $return_array['from']="     FROM notes  ";
            $return_array['where']="    where notes.deleted = 0 and emails.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
            $return_array['join'] = "   join emails on emails.id = notes.parent_id
                                    join aos_quotes on emails.parent_id = aos_quotes.id
                                    join leads_aos_quotes_1_c on leads_aos_quotes_1aos_quotes_idb = aos_quotes.id
                                    join leads on leads.id = leads_aos_quotes_1leads_ida
                                    join leads_cstm on leads_cstm.id_c = leads.id
                                    join accounts on leads_cstm.account_id_c = accounts.id";
            $return_array['join_tables'][0] = '';
    ...
    //note che vengono dai contatti dell'azienda
    public function get_notes_leads() {

        $return_array['select'] = " SELECT notes.id";
        $return_array['from']="     FROM notes  ";
        $return_array['where']="    where notes.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
        $return_array['join'] = "   join leads on leads.id = notes.parent_id
                                    join leads_cstm on leads_cstm.id_c = leads.id
                                    join accounts on leads_cstm.account_id_c = accounts.id";
        $return_array['join_tables'][0] = '';
    ...
    //email che vengono dai contatti dell'azienda
    public function get_emails_leads() {

        $return_array['select'] = " SELECT emails.id";
        $return_array['from']="     FROM emails  ";
        $return_array['where']="    where emails.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
        $return_array['join'] = "   join leads on leads.id = emails.parent_id
                                    join leads_cstm on leads_cstm.id_c = leads.id
                                    join accounts on leads_cstm.account_id_c = accounts.id";
        $return_array['join_tables'][0] = '';
    ...
    //meetings che vengono dai contatti dell'azienda con collegamento diretto (meetings->parent_id)
    public function get_meetings_leads_d() {

        $return_array['select'] = " SELECT meetings.id";
        $return_array['from']="     FROM meetings  ";
        $return_array['where']="    where meetings.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
        $return_array['join'] = "   join leads on meetings.parent_id = leads.id
    ...
    //meetings che vengono dai contatti dell'azienda con collegamento indiretto (invitati)
    public function get_meetings_leads_i() {

        $return_array['select'] = " SELECT meetings.id";
        $return_array['from']="     FROM meetings  ";
        $return_array['where']="    where meetings.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
        $return_array['join'] = "   join meetings_leads on meetings.id = meetings_leads.meeting_id
                                    join leads on meetings_leads.lead_id = leads.id
                                    join leads_cstm on leads.id = leads_cstm.id_c
                                    join accounts on accounts.id = leads_cstm.account_id_c";
        $return_array['join_tables'][0] = '';
    ...
    //calls che vengono dai contatti dell'azienda con collegamento diretto (calls->parent_id)
    public function get_calls_leads_d() {

        $return_array['select'] = " SELECT calls.id";
        $return_array['from']="     FROM calls  ";
        $return_array['where']="    where calls.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
        $return_array['join'] = "   join leads on calls.parent_id = leads.id
                                    join leads_cstm on leads_cstm.id_c = leads.id
                                    join accounts on accounts.id = leads_cstm.account_id_c";
        $return_array['join_tables'][0] = '';
    ...
    //call che vengono dai contatti dell'azienda con collegamento indiretto (tabella: crm.calls_leads)
    public function get_calls_leads_i() {

        $return_array['select'] = " SELECT calls.id";
        $return_array['from']="     FROM calls  ";
        $return_array['where']="    where calls.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
        $return_array['join'] = "   join calls_leads on calls.id = calls_leads.call_id
                                    join leads on leads.id = calls_leads.lead_id
                                    join leads_cstm on leads_cstm.id_c = leads.id
                                    join accounts on accounts.id = leads_cstm.account_id_c";
        $return_array['join_tables'][0] = '';
    ...
    //quotazioni relative a contatti dell'azienda
    public function get_quotes_leads() {

        $return_array['select'] = " select aos_quotes.id ";
        $return_array['from']="     from aos_quotes  ";
        $return_array['where']="    where aos_quotes.deleted = 0 and leads.deleted = 0 and accounts.deleted = 0 and accounts.id = '$this->id'";
        $return_array['join'] = "   join leads_aos_quotes_1_c on aos_quotes.id = leads_aos_quotes_1aos_quotes_idb
                                    join leads on leads.id = leads_aos_quotes_1leads_ida
                                    join leads_cstm on leads_cstm.id_c = leads.id
                                    join accounts on accounts.id = leads_cstm.account_id_c";
        $return_array['join_tables'][0] = '';
