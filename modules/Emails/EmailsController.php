...
class EmailsController extends SugarController
{
    ...
    public function action_send()
    {
        ...
        if(isset($request['return_module']) && isset($request['return_id']) && $request['return_module'] != '' && $request['return_id'] != ''){
            $request['parent_type'] = $request['return_module'];
            $request['parent_name'] = '';
            $request['parent_id'] = $request['return_id'];
        ...
        if ($mode === self::COMPOSE_BEAN_MODE_REPLY_TO || $mode === self::COMPOSE_BEAN_MODE_REPLY_TO_ALL) {
            ...
            if ($mode === self::COMPOSE_BEAN_MODE_FORWARD) {
                ...
                if ($mode === self::COMPOSE_BEAN_WITH_PDF_TEMPLATE) {
                    ...
                    $quote_id = $_GET['return_id'];

                    $query_quote_2_lead = "SELECT email_lead.id,first_name, last_name, email_address  FROM email_lead
                                           LEFT JOIN leads_aos_quotes_1_c ON email_lead.id = leads_aos_quotes_1_c.leads_aos_quotes_1leads_ida
                                           WHERE primario = '1' AND leads_aos_quotes_1aos_quotes_idb = '".$quote_id."'";
                    $result = $db->query($query_quote_2_lead,true);
                    ...
                    if ($result->num_rows == 0) {
                        $sql_account = "SELECT email_account.id, email_account.name, email_address  FROM email_account
                                           LEFT JOIN aos_quotes ON email_account.id = aos_quotes.billing_account_id
                                           WHERE primario = '1' and aos_quotes.id = '".$quote_id."' ";
                        $result2 = $db->query($sql_account,true);
                    ...
                    $this->bean->to_addrs_names = $parent_email;
